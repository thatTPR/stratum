...
void check_attrs_elem_with_values(fs::path attr_dir, fs::path target_file, vector<string> elems, vector<string> attrs)
{
    ...
    for (const auto file : fs::directory_iterator(attr_dir))
    {
        ...
        try
        {
            ...
            cout<<lines.substr(0,40)<<" ... "<<lines.substr(lines.length()-40,lines.length())<<"\n";// Print lines start and line end                 
            xml::xml_parse parser(xml::xml) ;
            xml::xmln xl =parser.from_str(lines);
            std::cout<<xl;
            // std::cout<<xl.els.front().name << " \n";
            for(int i ; i<xl.els.size();i++){std::cout<<xl.els[i]<<" ";}
            std::vector<xml::xmln*> article= xl.bfs_all([](xml::xmln* n){ std::string ar = "article"; return n->name==ar;});
            for(int i ; i<article.size();i++){cout<<&(article[i]);};
