...
namespace pri {
    ...
    */
        ...
        bool OneOf(T l){
            if(l==a){return true;}
            if constexpr (sizeof...(args)>0){return OneOf<T,a,args...>(l);}
            return false;
        };
...
template< Str h>
struct CEStr {
   static constexpr std::array<char,h.size> get_str(){return h.data;};
    static std::string str() {return h.str();}
};
...
struct opt {
    ...
    bool check(int& i,int argc,char* argv[]){
        ...
        else if constexpr (sizeof...(syns)>0){return opt<b,syns...>().check(str);}
        return false;
    };
};
...
*/
    ...
    template <typename A,typename Func,typename T,typename... Ts>
