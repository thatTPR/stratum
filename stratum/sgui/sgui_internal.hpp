...
void push_evptr(void (*ptr)(evT::dtype& p)) ;

template <even _E, typename T=void>
struct ev  {
    using dtype = T;
    static constexpr even evty = _E;
    using fptrty =  std::vector<void (*ptr)(dtype& p)> ; 
    void cb(dtype& p){};
    ev() {push_evptr<decltype(*this)>(&cb);};
};  
...
struct _evsys{
    ...
    NSC tuple<typename evTs::fptrty...> ptrs;

    template <typename evT>
    void push(void (*ptr)(evT::dtype& p)){NSC get<fptrt<evT>>(ptrs).push_back(ptr);}
    void pop(void (*ptr)(evT::dtype& p)){ 
        for(auto it  = NSC get<fptrt<evT>>(ptrs).begin() ; it != NSC get<fptrt<evT>>(ptrs).end() ; ++it  ){
            if(*it== ptr){NSC get<fptrt<evT>>(ptrs).erase(it);return;};
        }
    };

    
    template <typename evT , size_t s=0>
    auto& get(){
        if constexpr (sizeof...(evTs)>s) {
            if constexpr (std::is_same<evT::fptrty, decltype(NSC get<s>(ptrs))::fptrty >::value){return NSC get<s>(ptrs) ; }
            else {return get<evT,s+1>();}
        }
        assert( 0 , "Not Found evT ptr in ptrs") ;
    };

    template <typename evT>
    void proc(evT::dtype& p = NULL ) {for(auto& i : get<evT>() ) {i(p);}}
};
...
struct abort          :ev<even::_ev_abort          ,void>{void ev_abort(); void cb(){ev_abort();};};
struct activate       :ev<even::_ev_activate       ,void>{void ev_activate(); void cb(){ev_activate();};};
struct begin          :ev<even::_ev_begin          ,void>{void ev_begin(); void cb(){ev_begin();};};
struct cancel         :ev<even::_ev_cancel         ,void>{void ev_cancel(); void cb(){ev_cancel();};};
struct canplay        :ev<even::_ev_canplay        ,void>{void ev_canplay(); void cb(){ev_canplay();};};
...
struct mousedown      :ev<even::_ev_mousedown      ,impl::mousedown>{void ev_mousedown(dtype& dt, glm::vec2 mpos);void cb(dtype& dt){ev_mousedown(dt);};};
struct mouseenter     :ev<even::_ev_mouseenter     ,impl::mouse_move>{bool mouseon=false ;void ev_mouseenter(dtype& dt);void cb(dtype& dt){ev_mouseenter(dt);};};
struct mouseleave     :ev<even::_ev_mouseleave     ,void>{void ev_mouseleave();void cb(){ev_mouseleave(dt);};};
struct mousemove      :ev<even::_ev_mousemove      ,impl::mouse_move>{void ev_mousemove(dtype& dt);void cb(dtype& dt){ev_mousemove(dt);};};
struct mouseout       :ev<even::_ev_mouseout       ,void>{void ev_mouseout();void cb(){ev_mouseout(dt);};};
struct mouseover      :ev<even::_ev_mouseover      ,void>{void ev_mouseover();void cb(){ev_mouseover(dt);};};
struct mouseup        :ev<even::_ev_mouseup        ,impl::mouseup>{void ev_mouseup(dtype& dt,glm::vec mpos);void cb(dtype& dt){ev_mouseup();};};

struct click          : ev<even::_ev_click          ,void>{ ;void ev_click(impl::mousepos pos);void cb(dtype& dt){ev_click(sys.mouse.WinPos());};};
struct rclick         : ev<even::_ev_click          ,void>{void ev_rclick(impl::mousepos pos);void cb(dtype& dt){ev_rclick(sys.mouse.WinPos());};};
struct dblclick       : ev<even::_ev_dblclick       ,void>{void ev_dblclick(impl::mousepos);void cb(dtype& dt){ev_dbclick(sys.mouse.WinPos());};};
...
struct resize         :ev<even::_ev_resize         ,impl_resize>{bool resizingb=false;void resize_end(){resizingb=false;};
                                                                           virtual void eresizing(impl::mousemove& m){}; void ev_resize(){resizing=true;};void cb(dtype& dt){ev_resize();};};
...
#define ALL_EVS SVG_EVS,JOY_EVS,CONT_EVS,TOUCH_EVS,MOTION_EVS

_evsys<SVG_EVS,JOY_EVS,CONT_EVS,TOUCH_EVS> evsys;
template <typename evT>
void push_evptr(void (*ptr)(evT::dtype&)) {evsys.push<evT>(ptr);}
...
struct inherInhEvent : virtual InherT ,virtual InherTs... {
    temlate <ev enevt>
