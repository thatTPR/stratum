...
template <class shaderMod,class... shaderMods >
struct pipeline {
    using selftype = pipeline<shaderMod,shaderMods...> ;


    template <shader_type st>
    inline constexpr bool hasst = st==shaderMod::sty | pipeline<shaderMods...>::hasst<st> ;   
    template <shader_type st>
    using type = typename std::conditional<st==<shaderMod::sty,shaderMod ,typename pipeline<shaderMods...>::type<st>>::type ;


    template <typename sMod,typename... sMods>
    struct smod {
    ...
    #define BUFTYS(name) using SSBO#name = 
    using mod

    struct contribution {
        std::enable_if<hasst<shader_type::vert>,type<shader_type::vert>::type svert; 
        std::enable_if<hasst<shader_type::frag>,type<shader_type::frag>::type sfrag; 
        std::enable_if<hasst<shader_type::geom>,type<shader_type::geom>::type sgeom; 
        std::enable_if<hasst<shader_type::tesc>,type<shader_type::tesc>::type stesc; 
        std::enable_if<hasst<shader_type::tese>,type<shader_type::tese>::type stese; 
        std::enable_if<hasst<shader_type::comp>,type<shader_type::comp>::type scomp; 
        std::enable_if<hasst<shader_type::task>,type<shader_type::task>::type stask; 
        std::enable_if<hasst<shader_type::mesh>,type<shader_type::mesh>::type smesh; 
        std::enable_if<hasst<shader_type::rgen>,type<shader_type::rgen>::type srgen; 
        std::enable_if<hasst<shader_type::rint>,type<shader_type::rint>::type srint; 
        std::enable_if<hasst<shader_type::rahit>,type<shader_type::rahit>::type srahit; 
        std::enable_if<hasst<shader_type::rchit>,type<shader_type::rchit>::type srchit; 
        std::enable_if<hasst<shader_type::rmiss>,type<shader_type::rmiss>::type srmiss; 
        std::enable_if<hasst<shader_type::rcall>,type<shader_type::rcall>::type srcall; 
    };   
    NSC list<contribution> contribs;
